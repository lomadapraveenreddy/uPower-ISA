...
# special_reg={gp:'0x10008000',stp:'0x0000003FFFFFFFF0',pc:'0x0000000000400000'}
R = 'R'
r = 'r'
...
while i < len(lines):
    ...
    # instruction 33--------------
        ...
        if j - i - 1 > 0:
            ...
            instruction.append(bin((j - i - 1) & 2 ** 13 - 1)[2:])
        AA = 1
        LK = 0
...
def addi(instruction):
    if int(instruction[16]) == 0:
        ...
        reg_files[int(instruction[6:11], 2)] = reg_files[int(instruction[11:16], 2)] + int(instruction[17:],
                                                                                           2) - 2 ** 15
    return
...
def lwz(instruction):
    ...
    datafile.seek(0, 0)
    dataline = datafile.readline()
    stadd = int(reg_files[int(instruction[11:16], 2)]) + int(instruction[17:30], 2) - (2 ** 13) * int(
        instruction[16]) - data_addr
    if int(dataline[8 * stadd]) == 0:
        reg_files[int(instruction[6:11], 2)] = int(dataline[8 * stadd:8 * stadd + 32], 2)
    else:
        reg_files[int(instruction[6:11], 2)] = int(dataline[8 * stadd + 1:8 * stadd + 32], 2) - int(2 ** 31) * int(
            dataline[8 * stadd])
...
def lhz(instruction):
    ...
    global data_addr
    global dataline
    stadd = int(reg_files[int(instruction[11:16], 2)]) + int(instruction[17:30], 2) - (2 ** 15) * int(
        instruction[16]) - data_addr
